...
class Restropress_Menu_Cart_Loader {
	...
	public function menu_cart_deactivation() {
		...
		if ( ! is_plugin_active( 'restropress/restro-press.php' ) ) {
			...
			echo '<div id="notice" class="error"><p>' . sprintf( __( ' Restropress menu cart <a href="%1$s" target="_blank"> RestroPress </a> plugin to be installed. Please install and activate it', 'rp-restropress-menu-cart' ), esc_url( $plugin_link ) ).  '</p></div>';
	...
	public function menu_cart_settings_link(  $links ) {
		...
		$settings_link = sprintf( __( '<a href="%1$s">Settings</a>', 'rp-restropress-menu-cart' ), esc_url( $link ) );
